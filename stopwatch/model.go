// Copy of https://github.com/charmbracelet/bubbletea/blob/master/examples/stopwatch/main.go
// Modified to allow the setting of duration when initialising the stopwatch.
...
// interval.
func NewWithInterval(interval time.Duration, duration time.Duration) *Model {
	return &Model{
		d:        duration,
		Interval: interval,
		id:       nextID(),
...
func New(d time.Duration) *Model {
	return NewWithInterval(time.Second, d)
}
...
func (m *Model) Start() tea.Cmd {
	return tea.Batch(func() tea.Msg {
		log.Default().Print("Send start message")
...
func (m *Model) Update(msg tea.Msg) (*Model, tea.Cmd) {
	...
	case StartStopMsg:
		...
		log.Default().Print("Running: ", msg.running)
